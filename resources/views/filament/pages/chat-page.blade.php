...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Check for overdue rentals and send automatic notifications
            async function checkOverdueRentals() {
                try {
                    const response = await fetch('/chat/check-overdue-rentals', {
                        method: 'POST',
                        headers: {
                    ...
                    if (result.success && result.notifications_sent > 0) {
                        console.log(`Sent ${result.notifications_sent} overdue notification(s)`);
            ...
            // Check for delayed bookings and send automatic notifications
            async function checkDelayedBookings() {
                try {
                    const response = await fetch('/chat/check-delayed-bookings', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    ...
                    if (result.success && result.notifications_sent > 0) {
                        console.log(`Sent ${result.notifications_sent} delay notification(s)`);
                        // Refresh contacts to show new messages
                        loadContacts();
                    }
                } catch (error) {
            ...
            // Check for overdue rentals when admin opens the chat page
            checkOverdueRentals();

            // Check for delayed bookings when admin opens the chat page
