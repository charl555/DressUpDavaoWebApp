    ...
    <div
        ...
        <div class="w-full lg:w-1/4 flex flex-col mb-8 lg:mb-0">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
                ...
                <p class="text-gray-600 text-sm">Manage your account preferences and settings</p>
            </div>
        ...
        <div class="w-full lg:w-3/4">
            ...
            <div id="change-password" class="content-section hidden bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                <p class="text-gray-600 pb-6 text-sm">Update your account password.</p>
            ...
            <div id="delete-account" class="content-section hidden bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                <p class="text-gray-600 pb-6 text-sm">Permanently delete your account. This action cannot be undone.</p>
                ...
                @if(Auth::user()->deletion_requested_at)
                    <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 mb-6">
                        ...
                        <form method="POST" action="{{ route('account.cancel-deletion') }}" class="mt-4">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="px-6 py-2 bg-gray-600 text-white rounded-lg font-semibold hover:bg-gray-700 transition-all duration-300">
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <button type="button" @click="showForm = true"
                                class="px-6 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 shadow-md">
                                Add Measurements
                            </button>
                        </div>
                    @endif
                    ...
                    <form @submit.prevent="submitMeasurements" class="space-y-6">
                        ...
                        <div class="flex justify-end space-x-4 pt-4">
                            <button type="button" @click="showForm = false"
                                class="px-6 py-2 border-2 border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 hover:border-gray-400 transition-all duration-200">
                                Cancel
                            </button>
                        ...
                        <div x-show="generalError" class="rounded-xl bg-red-50 p-4 border border-red-200">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-red-700 mt-1" x-text="generalError"></p>
                                </div>
                            </div>
                        </div>
    ...
    <form id="preferencesForm" class="space-y-6">
        ...
        <div class="pt-8 flex justify-end">
            ...
            <div id="my-favorites" class="content-section hidden bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                <p class="text-gray-600 pb-6 text-sm">View and manage your favorite products.</p>
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex justify-between items-center mb-3">
                                        <span class="text-sm text-gray-500 font-medium">
                                            {{ $product->user->shop->shop_name ?? 'No Shop' }}
                                        </span>
                                    </div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 relative transform transition-all duration-300 scale-100">
        ...
        <div class="text-center">
            <h3 class="text-xl font-bold text-gray-800 mb-3">Confirm Account Deletion</h3>
        ...
        <div class="flex justify-center space-x-4">
            ...
            <button type="button" onclick="submitDeletionRequest()"
                class="px-6 py-2 bg-gradient-to-r from-red-600 to-pink-600 text-white rounded-lg font-semibold hover:from-red-700 hover:to-pink-700 transition-all duration-300 shadow-md">
...
<div id="reviewModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 relative transform transition-all duration-300 scale-100">
        ...
        <h2 class="text-xl font-bold text-gray-800 mb-2 text-center">
            <span id="modalActionLabel">Leave</span> a Review for
            <span id="modalShopName" class="text-purple-600"></span>
        </h2>
        <p id="modalBookingCount" class="text-sm text-gray-500 text-center mb-6"></p>

        <!-- Review Form -->
        <form id="reviewForm" method="POST" action="{{ route('user.submitReview') }}" class="space-y-6">
            @csrf
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const activeTab = localStorage.getItem('activeTab') || 'profile-settings';

        // Set active tab on load
        sections.forEach(sec => {
            sec.classList.toggle('active', sec.id === activeTab);
            sec.classList.toggle('hidden', sec.id !== activeTab);
        });
    ...
    function measurementsForm() {
        return {
            ...
            validateField(field) {
                ...
                switch (field) {
                    case 'chest': min = 20; max = 60; errorMsg = 'Chest must be between 20–60 inches.'; break;
                    case 'waist': min = 20; max = 50; errorMsg = 'Waist must be between 20–50 inches.'; break;
                    case 'hips': min = 20; max = 60; errorMsg = 'Hips must be between 20–60 inches.'; break;
                    case 'shoulder': min = 10; max = 30; errorMsg = 'Shoulder must be between 10–30 inches.'; break;
                    default: return;
                }
            ...
            async submitMeasurements() {
                ...
                try {
                    const response = await fetch("{{ route('measurements.update') }}", {
                        method: "POST",
                        headers: {
    ...
    function openReviewModal(shopId, shopName, isUpdate = false) {
        document.getElementById('modalShopId').value = shopId;
        document.getElementById('modalShopName').innerText = shopName;
        document.getElementById('modalActionLabel').innerText = isUpdate ? 'Update' : 'Leave';

        fetch(`/user/review-data/${shopId}`)
            .then(res => res.json())
            .then(data => {
                document.getElementById('modalBookingCount').innerText =
                    `(${data.bookingCount} ${data.bookingCount > 1 ? 'purchases' : 'purchase'})`;

                document.getElementById('modalRating').value = data.review?.rating || '';
                document.getElementById('modalComment').value = data.review?.comment || '';
            });
